...
void MAMWetscav::run_impl(const double dt) {
  ...
  Kokkos::parallel_for(
      policy, KOKKOS_LAMBDA(const ThreadTeam &team) {
        ...
        // FIXME: need these initialized
        Real scavimptblvol[mam4::aero_model::nimptblgrow_total]
                          [mam4::AeroConfig::num_modes()] = {{0.0}};
        Real scavimptblnum[mam4::aero_model::nimptblgrow_total]
                          [mam4::AeroConfig::num_modes()] = {{0.0}};
        init_scavimptbl(scavimptblvol[aero_model::nimptblgrow_total]
                                     [AeroConfig::num_modes()],
                        scavimptblnum[aero_model::nimptblgrow_total]
                                     [AeroConfig::num_modes()]);
